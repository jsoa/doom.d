...
A changed row is also allowed to **run onto continuation rows** rather than being cut off at the cell width, leaving the name column blank so the field still reads as one entry. On that row the definition is the thing you came to read, so it is spelled out in full (up to `expose-migrations-max-detail-lines`), while the unchanged rows around it — carried forward from the migration before, and context rather than news — still truncate at `expose-migrations-max-definition-width`.
