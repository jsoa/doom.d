...
It is an ordinary, read-only Evil-normal-state buffer -- navigate, search, and yank in it the same as anywhere else: `yy`, `y$`, `yiw`, a Visual-state `y` over a selection, all work normally, so copying just part of the result is nothing special. Extra keys:
...
**The ER diagram sees more than the current file.** A model's own source only ever shows what it points *outward* to — its own `ForeignKey`/`OneToOneField`/`ManyToManyField` declarations are right there in its class body. What points *inward* — another app's model with a `ForeignKey(ThisModel, ...)` of its own — is invisible from here, and routinely lives in a completely different file. Before asking a provider anything, this greps the whole project for a relationship field naming the model at point as its target (both the bare-class form and the quoted string forms Django itself supports to avoid circular imports), confirms each match is genuinely inside one of those three field calls, and folds the real declaring model's full body into the request. It's drawn as a complete `shape=Mrecord` node with its own real fields, the same as a model actually defined in the current file — not the name-only `shape=component` box everything else genuinely external gets. A self-referential field (a `parent` FK naming its own model) is excluded, and so is a model already in the same file being diagrammed, since that one is already part of the ordinary code sent. Capped to `expose-relations-max-models` (8) models and `expose-relations-max-model-length` (4000) characters each.

**It also resolves the model's own base classes.** A mixin (`TimestampedModel`, `SoftDeleteModel`) is named right there in the `class Event(...):` header, but routinely *defined* somewhere else — a shared file, the whole point of a mixin. Each base name is searched for under the project root the same way, and when its definition is found exactly once, its full body is folded in and drawn as a real `shape=Mrecord` node too, connected to the model with a plain UML-style generalization edge (`arrowhead=empty`, dashed). Django's own `Model` and Python's own `object` are skipped before searching, and an ambiguous name (found more than once) drops out rather than guessing which definition is the real base. This is the one thing about the diagram Expose colors on its own rather than leaving to the provider: the base classes get a distinct, fixed color — but only for the model you invoked the command from, not for every model in the diagram. Capped to `expose-relations-max-base-classes` (4).
