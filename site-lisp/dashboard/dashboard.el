...
(defface jsoa/diag-error-button
  '((t (:inherit error :underline t)))
  "Error button face.")

(defface jsoa/diag-warning-button
      ...
      (let ((inhibit-read-only t))
        ...
        (let* ((filtered
                (if severity
                    (seq-filter
                     (lambda (d)
                       (equal (plist-get d :severity) severity))
                     diagnostics)
                  diagnostics))

               (errors (seq-count (lambda (d)
                                    (equal (plist-get d :severity) "error"))
                                  diagnostics))

               (warnings (seq-count (lambda (d)
                                      (equal (plist-get d :severity) "warning"))
                                    diagnostics)))

          ;; title
          (insert (format "%s (%d errors, %d warnings)\n"
                          (or title "Diagnostics")
                          errors warnings))
          ...
          (let ((groups (make-hash-table :test 'equal)))
            ...
            (dolist (d filtered)
              (let ((file (plist-get d :file)))
                (puthash file (cons d (gethash file groups)) groups)))

            (dolist (file (sort (hash-table-keys groups) #'string<))
              (let* ((items (reverse (gethash file groups)))
                     (short (jsoa/short-path file root)))

                ;; file header
                (insert (propertize short 'face 'bold))
                (insert "\n")
                ...
                (dolist (d items)
                  (let* ((line (plist-get d :line))
                         ...
                         (face (if (equal sev "error") 'error 'warning))
                         (start (point)))
...
(defun jsoa/ts-diagnostics-section (root)
  (when (memq (jsoa/project-type root) '(angular node))
    ...
    (let* ((dashboard-buf (current-buffer))
      ...
      (make-process
       :name "jsoa-tsc"
       :buffer (generate-new-buffer " *jsoa-tsc*")
       :command
       (list "npx" "tsc" "--noEmit" "--pretty" "false"
             "-p"
             (cond
              ((file-exists-p (expand-file-name "tsconfig.app.json" root))
               "tsconfig.app.json")
              ((file-exists-p (expand-file-name "tsconfig.json" root))
               "tsconfig.json")
              (t "tsconfig.json")))
       :noquery t
       :sentinel
...
shared with `jsoa/git-summary-section' instead of being re-fetched here."
  (let* ((name (file-name-nondirectory (directory-file-name root)))
    ...
    ;; Size -- `du -sh .' recursively walks the whole project tree, which
    ;; on a big node_modules/.git history, or a slow/networked filesystem,
    ;; can take anywhere from seconds to tens of seconds. Run it the same
    ;; way LOC/TODOs/diagnostics already are (`make-process' + sentinel,
    ;; guarded by `jsoa/dashboard-render-token') instead of blocking the
    ;; whole of Emacs on `shell-command-to-string' for however long that
    ;; walk takes.
    (insert "Size:    ")

    (let* ((dashboard-buf (current-buffer))
           (token jsoa/dashboard-render-token)
           (size-start (point-marker))
           size-end)

      (insert "Scanning...")
      (setq size-end (point-marker))
      (insert "\n\n")

      (make-process
       :name "jsoa-du"
       :buffer (generate-new-buffer " *jsoa-du*")
       :command (list shell-file-name shell-command-switch "du -sh . 2>/dev/null | cut -f1")
       :noquery t
       :sentinel
       (lambda (p _event)
         (when (memq (process-status p) '(exit signal))
               ...
               (with-current-buffer dashboard-buf
                 (let ((inhibit-read-only t))
                   (save-excursion
                     ...
                     (delete-region size-start size-end)
                     (insert
                      (if (string-empty-p output) "N/A" output)))))))))))
...
(defun jsoa/dashboard-actions-section (root start-index)
